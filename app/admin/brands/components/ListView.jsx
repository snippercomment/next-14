            ...
            <table className="border-separate border-spacing-y-3">
                <thead>
                    <tr>
                        ...
                        <th className="font-semibold border-y bg-white px-3 py-2 text-left">Tên thương hiệu</th>
...
function Row({ item, index }) {
    ...
    const handleDelete = async () => {
        if (!confirm("Bạn có chắc chắn muốn xoá thương hiệu này không?")) return;
        setIsDeleting(true);
        try {
        ...
        <tr>
            ...
            <td className="border-y bg-white px-3 py-2 text-center" style={{ verticalAlign: 'middle' }}>
                <div className="flex justify-center">
                    <img src={item?.imageURL} alt={item?.name} className="h-10 object-cover" />
                </div>
            </td>
