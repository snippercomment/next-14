...
export default function Page({ categoryFilter = null, params }) {
  ...
  const getCategoryInfo = () => {
    ...
    for (const parent of categories) {
      if (parent.children) {
        const corsairSubCat = parent.children.find(child => {
          const name = child.name?.toLowerCase() || '';
          return name.includes('corsair');
        });
        ...
        if (corsairSubCat) {
          return {
            ...
            subCategoryId: corsairSubCat.id,
            subCategoryName: corsairSubCat.name
          };
        }
      }
    }
  ...
  const products = allProducts?.filter(product => {
    ...
    const isLaptop = categoryName.includes('corsair') || 
                 
                   product.type === 'corsair' ||
                   product.productType === 'corsair' ||
                   product.category === 'corsair';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <CommentsSection 
              ...
              subCategoryName={categoryInfo.subCategoryName || "Corsair"}
