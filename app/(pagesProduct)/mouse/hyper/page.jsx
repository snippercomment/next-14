...
export default function Page({ categoryFilter = null, params }) {
  ...
  const getCategoryInfo = () => {
    ...
    for (const parent of categories) {
      if (parent.children) {
        const hyperSubCat = parent.children.find(child => {
          const name = child.name?.toLowerCase() || '';
          return name.includes('hyper');
        });
        ...
        if (hyperSubCat) {
          return {
            ...
            subCategoryId: hyperSubCat.id,
            subCategoryName: hyperSubCat.name
          };
        }
      }
    }
  ...
  const products = allProducts?.filter(product => {
    ...
    const isLaptop = categoryName.includes('hyper') || 
                 
                   product.type === 'hyper' ||
                   product.productType === 'hyper' ||
                   product.category === 'hyper';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
     ...
     <CommentsSection 
             ...
             subCategoryName={categoryInfo.subCategoryName || "Hyper"}
