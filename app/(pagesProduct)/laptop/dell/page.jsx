...
export default function Page({ categoryFilter = null, params }) {
  ...
  const findCategoryById = (categoryId) => {
   ...
   const getCategoryInfo = () => {
    ...
    for (const parent of categories) {
      if (parent.children) {
        const dellSubCat = parent.children.find(child => {
          const name = child.name?.toLowerCase() || '';
          return name.includes('dell');
        });
        ...
        if (dellSubCat) {
          return {
            ...
            subCategoryId: dellSubCat.id,
            subCategoryName: dellSubCat.name
          };
        }
      }
    }
  ...
  const products = allProducts?.filter(product => {
    ...
    const isLaptop = categoryName.includes('dell') || 
                   categoryName.includes('dell') ||
                   product.type === 'dell' ||
                   product.productType === 'dell' ||
                   product.category === 'dell';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <CommentsSection 
             ...
             subCategoryName={categoryInfo.subCategoryName || "Dell"}
