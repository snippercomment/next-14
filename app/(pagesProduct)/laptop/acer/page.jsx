...
export default function Page({ categoryFilter = null, params }) {
  ...
  const findCategoryById = (categoryId) => {
   ...
   const getCategoryInfo = () => {
    ...
    for (const parent of categories) {
      if (parent.children) {
        const acerSubCat = parent.children.find(child => {
          const name = child.name?.toLowerCase() || '';
          return name.includes('acer');
        });
        ...
        if (acerSubCat) {
          return {
            ...
            subCategoryId: acerSubCat.id,
            subCategoryName: acerSubCat.name
          };
        }
      }
    }
  ...
  const products = allProducts?.filter(product => {
    ...
    const isLaptop = categoryName.includes('acer') || 
                 
                   product.type === 'acer' ||
                   product.productType === 'acer' ||
                   product.category === 'acer';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
     ...
     <CommentsSection 
        ...
        subCategoryName={categoryInfo.subCategoryName || "Acer"}
