...
export default function Page({ categoryFilter = null, params }) {
  ...
  const findCategoryById = (categoryId) => {
   ...
   const getCategoryInfo = () => {
    ...
    for (const parent of categories) {
      if (parent.children) {
        const lenovoSubCat = parent.children.find(child => {
          const name = child.name?.toLowerCase() || '';
          return name.includes('lenovo');
        });
        ...
        if (lenovoSubCat) {
          return {
            ...
            subCategoryId: lenovoSubCat.id,
            subCategoryName: lenovoSubCat.name
          };
        }
      }
    }
  ...
  const products = allProducts?.filter(product => {
    ...
    const isLaptop = categoryName.includes('lenovo') || 
                 
                   product.type === 'lenovo' ||
                   product.productType === 'lenovo' ||
                   product.category === 'lenovo';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
     ...
     <CommentsSection 
                 ...
                 subCategoryName={categoryInfo.subCategoryName || "Lenovo"}
